...
const Note_TEMPLATES = [
  (c) =>
    ` Discovery call with ${c} went well - strong interest in the analytics module.`,
  (c) =>
    ` ${c} flagged budget concerns on the Enterprise tier. Prepare an ROI one- pager before the next call.`,
  (c) =>
    ` Champion at ${c} is pushing internally, legalreview is the main blocker right now.`,
  (c) =>
  ...
  (c) =>
    ` ${c} comparing us against a competitior on price. Emphasise support SLA and onboarding`,
  (c) =>
  ...
  (c) =>
    ` Procument at ${c} confirmed budget. Moving to contract readlines this week.`,
];

const TASK_TEMPLATES = [
  (c) => `Send proposal follow-up to ${c} `,
  (c) => `Schedule technical deep-dive with ${c} `,
  (c) => `Quaterly check-in with ${c} `,
  (c) => `Draft ROI one-pager for ${c} `,
  (c) => `Share case study with ${c} `,
  (c) => `Confirm contract readlines with ${c} `,
...
const run = async () => {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    leadDocs.push({
      ...
      notes: pick([
        "Imboud from website demo request.",
