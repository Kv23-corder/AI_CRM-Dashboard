...
const generateJSON = async (prompt, schema) => {
  ...
  try {
    ...
    throw new ApiError(502, "AI request failed. Please try again in a momnet.");
  }
};
...
export const generateLeadSummary = async (lead) => {
  ...
  const schema = {
    ...
    properties: {
      ...
      riskScore: {
        type: "integer",
        description: "Risk of losing this deak, 0 (safe) to 100 (high risk)",
      },
      suggestedPriority: {
        type: "string",
    ...
    requied: ["summary", "riskScore", "suggestedPriority", "nextBestAction"],
  };
...
export const generateSalesInsights = async (pipelineStats) => {
  ...
  const schema = {
    ...
    properties: {
      headline: {
        type: "string",
        description: "One-sentense summary of pipeline health",
      },
      insights: {
        type: "array",
